...
Отправляется, когда участник открыл окно чата для получения сообщений.
...
В ответ сервер отправит этот же ивент с массивом сообщений:
...
Отправляется, когда участник отправляет сообщение в чат.
...
| `error.chat` | Ошибки чата | В случае отсутствия параметров `content` или `type` |